...
function Projects({ containerRef, closeApplication }) {
  const getHeader = () => {
    return (
      <div className="flex items-center">
        <ActionBar
          closeApplication={closeApplication}
          currentApplication={Application_constants.Projects}
        />
        <div className="mx-auto text-sm font-medium px-2 py-1">
          <img
            ...
            src={ResumeIcon}
          />
          <span className="inline-block align-middle">Anil Kumar - - zsh</span>
        </div>
      </div>
    );
  };
